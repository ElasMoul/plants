...
> (a) `../AGENTS.md` (the platform's standing orders) takes precedence over
...
> platform handoff file for delta sessions — PlantPal's own `.Codex/STATE.md`
...
> ## Demand system
> Platform-delta only; platform standing orders in `../AGENTS.md` take precedence
...
# PlantPal — Codex Instructions

> This file is read automatically by Codex on every session.
...
└── .Codex/                 Agent memory — AGENTS, ARCHITECT, BACKEND, FRONTEND,
                              STATE, TASK_PLAN, AGENTS.md (this file)
```
...
| AnthropicClient | Codex-sonnet-4-6 | Vision (identification + annotation) AND reasoning (cure advice, disease description) — Codex is multimodal, one client serves both menus. `anthropic.api.key` has no required default; `isAvailable()` gates the option off when un-keyed (see `UserPreferencesResponse.visionModelAvailability`/`reasoningModelAvailability`) — `VisionModelPreference`/`ReasoningModelPreference.ANTHROPIC_CLAUDE` |
...
> Archived snapshots: .Codex/Archive/.
...
| I18N explicit sections | Complete on `codex/explicit-section-translations` (2026-09-24): UI language switching reuses saved target text automatically, missing translations stay explicit, new generation uses account language, manual saved EN/FR/AR section variants with contextual translate icons beside read-aloud (no dropdowns), no global banner/read-triggered translation, section-matched speech; migration 039; owner testing pending |
...
- **Doc sync is the last step of every phase implementation — always, automatically.**
  After the final code commit, update the `.Codex/` files (STATE.md, AGENTS.md build
  ...
  sessions additionally append their handoff to `PROGRESS.md` per `../AGENTS.md`.
  (The former external `plants-vault` was removed 2026-07-15 — `.Codex/` is the
