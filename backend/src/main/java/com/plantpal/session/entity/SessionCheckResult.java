 ...
 * validate-and-slide check (renews the idle window) or a read-only status peek (does not). {@code
 * valid=false} always carries a {@link RevokedReason}; the registry infers IDLE_TIMEOUT vs
 * ABSOLUTE_CAP for a naturally-expired session (see SessionRegistryServiceImpl's class Javadoc for
 * why a TTL eviction can't leave its own tombstone) and reads the explicit reason back for a
 * revoked one.
 */
@Getter
